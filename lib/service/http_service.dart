...
class HttpService {
  Future<List<Ong>> fetchOngs() async {
    print("fetching ongs . . .");
    final response =
        await get(Uri.parse("http://192.168.0.107:8080/api/ong/list"));
  ...
  Future<Ong> saveOng(Ong ong) async {
    ...
    final response = await post(
        Uri.parse("http://192.168.0.107:8080/api/ong/register"),
  ...
  Future<Ong> findOngByEmail(String email) async {
    ...
    final response =
        await get(Uri.parse("http://192.168.0.107:8080/api/ong/" + email));
  ...
  Future<Ong> updateOng(Ong ong, String id) async {
    ...
    final response = await put(
        Uri.parse("http://192.168.0.107:8080/api/ong/update/" + id),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode(ong));
